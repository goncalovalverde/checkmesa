...
// All-or-nothing: tables, VAT rates, categories, and the admin user are created
// in a single transaction so a partial failure leaves the DB empty and re-runnable.
...
async function main() {
  ...
  console.log(
    ...
    `${CATEGORIES.length} categorias, ${TABLES.length} mesas criadas`
  );
}
